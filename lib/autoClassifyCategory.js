/**
 * 内容分类（category）：关键词权重，与 scripts/classify_all_poems.js 算法一致。
 * 供 server 启动时对「未填 category」行补全，避免仅 JSON 种子时分类全空。
 */
  ...
  for (const [category, keywords] of Object.entries(CATEGORY_KEYWORDS)) {
    ...
    for (const { kw, w } of keywords) {
      ...
      if (totalMatches > 0) {
        ...
        score += w * (1 + titleBonus) * Math.min(totalMatches, 3);
      }
    }
...
/** 去掉首尾空白（含全角空格 U+3000、零宽字符等），用于展示与「是否为空」判断 */
function trimCategoryDisplay(value) {
  if (value == null || value === '') return '';
  return String(value).replace(/^[\s\u00A0\uFEFF\u3000\u200B-\u200D]+|[\s\u00A0\uFEFF\u3000\u200B-\u200D]+$/g, '');
}

/** 视为「未填内容分类」：NULL、空串、仅空白/全角空格等（SQLite 的 TRIM 认不全这些） */
function isBlankCategory(value) {
  return trimCategoryDisplay(value).length === 0;
}
 ...
 */
function applyContentCategoriesToEmpty(db) {
  const all = db.prepare('SELECT id, title, content, category FROM poems').all();
  const rows = all.filter((row) => isBlankCategory(row.category)).map(({ id, title, content }) => ({ id, title, content }));
  if (!rows.length) return 0;
  const upd = db.prepare('UPDATE poems SET category = ?, category_score = ? WHERE id = ?');
  db.transaction(() => {
    for (const row of rows) {
      const r = classifyPoem(row.title, row.content);
      const cat = trimCategoryDisplay(r.category) || '诗词文苑';
      upd.run(cat, r.score, row.id);
    }
  })();
  return rows.length;
}
